<template>
	<view class="content">
		<view class="new-content">
			<view v-if="article">
				<view class="news-list" v-for="item in article" @click="helpOneDetail(item.help_id)">
					<view class="news-item-img">
						<image :src="item.help_img"></image>
					</view>
					<view class="news-item-content">
						<view class="news-item-content-title">{{item.help_title}}</view>
	...
	export default {
		...
		methods: {
			...
			getHelpList:function(){
				...
				t.urlRequest.urlRequest(t.func.getHelpList,data,function(res){
					...
					for (var i = 0; i < res.data.help_list.length; i++) {
						...
						t.article.push(res.data.help_list[i]);
					}
					uni.hideLoading();
				})
			...
			initData(){
				...
				t.urlRequest.urlRequest(t.func.getHelpList,data,function(res){
					...
					for (var i = 0; i < res.data.list.length; i++) {
						if(res.data.list[i]['help_img']!=null){
							res.data.list[i]['help_img']=t.sta.sitebase+res.data.list[i]['help_img'];
						}else{
							res.data.list[i]['help_img']='../../static/img/logo.jpg';
						}
					}
...
<style>
	...
	.news-list{width: 100%;height: 220upx;display: flex;margin:10upx auto;align-items: center;border-bottom: 1px solid #e8e8e8;}
	.news-list:last-child{border-bottom: 0upx;}
	...
	.news-item-content{flex: 7;height: 180upx;padding-left: 20upx;}
	...
	.news-item-content-time{margin-top: 30upx;height: 30upx;text-align: right;color: #B3B3B3;font-size: 12px;}
</style>
