<template>
	<view class="content">
		...
		<view v-if="list">
			<view class="list-item" v-for="item in list" @click="goDetail(item.worklog_id)">
				<view class="list-title">
					<view class="list-title-text">{{item.worklog_title}}</view>
					<view class="list-title-time">{{item.worklog_date}}</view>
				</view>
				<view class="list-content">{{item.worklog_audit_content}}</view>
				<view class="list-bottom">
					<view class="list-checker">审核人: {{item.worklog_audit_man}}</view>
					<view class="list-type">类型：{{item.worklog_type}}</view>
				</view>
...
<style>
	...
	.list-title-text{flex: 4;padding: 10upx 0;font-weight: bold;}
	.list-title-time{flex: 1;font-size: 12px;color: red;height: 30upx;line-height: 30upx;margin-top: 10upx;text-align: center;overflow: hidden;}
	...
	.list-type{flex:2;line-height: 30upx;padding: 10upx;color:#B3B3B3;border-radius: 10upx;font-size: 12px;text-align: right;}
</style>